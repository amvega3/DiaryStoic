        ...
        <div class="m-3 p-1 shadow-cyan-200 w-full mx-auto overflow-hidden bg-transparent rounded-lg shadow min-w-max-content">
            <div class="max-w-7xl px-2 mx-auto mb-1 overflow-hidden bg-transparent ">
                ...
                <div x-show="open">
                    ...
                    @if ($accion == 'store')
                        <div class="bg-transparent text-slate-200  m-2 p-2  shadow-md text-center">
                            ...
                            <tbody class="items-center w-full mx-auto divide-y divide-gray-300">
                                @foreach ($roles as $role)
                                    <tr class="text-xs  text-justify mx-auto p-2 text-gray-100">
                                        <td class="px-4 py-3 text-sm  text-justify">{{ $role->name }}</td>
                                        <td class="px-4 py-3 text-sm  text-justify">{{ $role->description }}</td>
                                        <td class="px-4 py-3 text-sm whitespace-nowrap text-justify">
                                            <button wire:click="edit({{ $role }})" x-on:click="open = !open , for_edit = !for_edit , edit = !edit"
                                                class="btn border-[#FFC600] hover:text-black text-white text-sm bg-transparent border px-4 py-1 uppercase tracking-tighter rounded hover:bg-[#FFC600]">EDITAR</button>
                                            <button wire:click="asignar({{ $role }})"
                                                class="btn border-[#FFC600] hover:text-black text-white text-sm bg-transparent border px-4 py-1 uppercase tracking-tighter rounded  hover:bg-[#FFC600]">PERMISOS</button>

                                            <button wire:click="destroy({{ $role }})"
                                                class="btn border-[#FFC600] hover:text-black text-white text-sm bg-transparent border px-4 py-1 uppercase tracking-tighter rounded  hover:bg-[#FFC600]">ELIMINAR</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
